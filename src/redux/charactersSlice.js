...
export const fetchCharacters = createAsyncThunk(
  ...
  async (page) => {
    // async şu işe yarar: fetchCharacters fonksiyonu çalıştığında, async sayesinde, fetchCharacters fonksiyonu içindeki kodlar çalışır. fetchCharacters fonksiyonu içindeki kodlar çalıştıktan sonra, fetchCharacters fonksiyonu döndürür.
...
export const charactersSlice = createSlice({
  ...
  extraReducers: {
    [fetchCharacters.pending]: (state, action) => {
      // pending: işlem başladığında çalışır.
      state.status = "loading"; // status componentini göster.
    },
    [fetchCharacters.fulfilled]: (state, action) => {
      // fulfilled: işlem başarılı olduğunda çalışır.
      ...
      if (action.payload.length < 12) {
        // action.payload'ın içindeki eleman sayısı, char_limit'ten küçükse, daha fazla sayfa yok demektir.
        state.hasNextPage = false; // daha fazla sayfa yoksa, hasNextPage'i false yaparak, daha fazla sayfa yok demek oluyor.
      }
    },
    [fetchCharacters.rejected]: (state, action) => {
      // rejected: işlem başarısız olduğunda çalışır.
